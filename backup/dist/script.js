...
function setAnimationToSlide(slideNode, animationParams) {
  const { animationDuration, animationType } = animationParams;
...
function setSlideIntoUI(slide, animationParams) {
  const { animationDuration, animationType } = animationParams;

  const newSlideNode = document.createElement("span");
  newSlideNode.textContent = slide;

  const animatedSlideNode = setAnimationToSlide(newSlideNode, {
    animationDuration,
    animationType });
...
function removeSlideFromUI(animationParams) {
  const { slideDuration, animationDuration, animationType } = animationParams;
...
function switchSlides(slides, animationParams) {
  const { slideDuration, animationDuration, animationType } = animationParams;
